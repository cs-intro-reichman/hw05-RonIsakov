...
public class GameOfLife {
	...
	private static void test2(String fileName) {
		int[][] board = read(fileName);
		int status = cellValue(board, 3, 2); 
		boolean alive  = false;
		if (status == 1) {
			alive = true;
	...
	public static int[][] evolve(int[][] board) {
		...
		for(int i=1 ; i<nextBoard.length-1 ; i++){
			for(int j=1 ; j<nextBoard[0].length-1 ; j++){
	...
	public static int cellValue(int[][] board, int i, int j) {
		int neighbotStatus =(count(board,i,j));
		boolean alvie = board[i][j] == 1;
		if((neighbotStatus < 2 || neighbotStatus > 3) && alvie){
		return 0;
		}
	else if(neighbotStatus == 3){
		return 1;
	}
	else{
		return board[i][j];
	...
	//cheking if the squaer that sourunds the cell in palce i,j is live or dead counting
	//cheking if the cell is not the cell i,j
	...
	public static int count(int[][] board, int i, int j) {
		int count = 0;
		//checking the status live/dead of each of the  neighbors
		for(int k=0; k<3 ; k++) {
